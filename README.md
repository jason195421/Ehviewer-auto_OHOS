...
最新安装包：[`EhViewer_OHOS_0.5.4.hap`](https://github.com/suibianqwe/Ehviewer_OHOS/releases/download/v0.5.4/EhViewer_OHOS_0.5.4.hap)<br />
发布包类型：未签名 HAP  
目标API：`26.0.0`  
兼容API：`6.0.0(20)`  
...
- 阅读：支持内嵌/独立阅读器、连续阅读、缩放、方向适配、进度控制、自适应一屏双页和本地下载优先读取，宽屏状态下始终保持独立全屏。
- 下载：支持任务暂停/继续/删除、多线程下载、本地阅读、通知栏进度与速度、隐私通知、恢复下载项和导出压缩包；可识别存档、应用导出包及外来画廊 ZIP 并重建元数据。
- 设置：支持简体中文、繁体中文、明暗主题、多种主题色、启动页、下载目录、过滤规则、代理/Hosts/域名前置、DoH、标签翻译库更新、隐私防护和身份验证。
- 翻译：支持列表标题、详情标题与评论翻译，可选网页翻译、DeepSeek、OpenAI、Gemini 和自定义兼容 API，并支持可编辑系统提示词、可视区域优先和合并翻译请求。
...
- 阅读器新增自适应一屏双页，可按屏幕与图片方向自动将相邻页面组合显示，并扩大相邻页预渲染范围。
- 翻译新增 AI 翻译开关、系统提示词预设与自定义管理、可视区域优先调度和合并请求；AI 每批最多合并 5 条且保持并发。
- 完善混合语言、罗马音及括号内容翻译，优化提示词缓存结构、结果完整性检查和失败重试。
- 完善 DoH 配置与直连检测，直连结果仅控制 Hosts 和 SNI 前置，测速仅在 SNI 前置启用时生效。
- 新增繁体中文、天蓝等多种主题色，标题栏和启动过渡背景跟随主题；标签翻译数据库更新并支持在关于页在线检查更新。
- 修复退出登录后 Cookie 未完全清理、登录页面小屏布局异常、模型列表获取后未及时刷新、网页翻译偶发失败和冷启动首屏缩略图无法加载的问题。

## 0.5.3 重点变化

- 完善翻译服务设置、模型自动获取、请求结果提示和自定义 API 管理。
- 完善 HTTP/SOCKS5 代理配置浮窗，SOCKS5 支持认证、排除列表与代理端 DNS，并对低 API 设备自动禁用。
- 翻译请求仅遵循代理设置，关闭翻译显示时会中断当前及排队中的翻译请求。
- 优化 DeepSeek V4 翻译延迟和超时处理，修复模型抓取时可能读取不到刚输入密钥的问题。
- 统一画廊卡片空星样式，使用实心灰色底星。
