...
本教程适用于 EhViewer HarmonyOS `0.5.5`。不同设备的系统版本、窗口尺寸和权限状态可能让按钮位置略有差异，但功能入口和操作逻辑相同。
...
- 导出数据：将设置、历史、下载信息、收藏书签和过滤规则保存为 JSON。
- 导入数据：读取本应用备份或支持的原安卓应用数据库。
...
可传输搜索书签、本地收藏、下载元数据和已下载图片。传输时保持两台设备页面开启，完成前不要主动断开 Wi-Fi 直连。
